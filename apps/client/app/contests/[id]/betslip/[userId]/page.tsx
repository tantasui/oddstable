...
type Pick = {
  ...
  result: string | null;
  fixture: { homeTeam: string; awayTeam: string; kickoff: string; status: string };
};
...
export default async function BetslipPage({
  ...
  const [entryRes, contestRes, userRes] = await Promise.all([
    fetch(`${SERVER}/api/contests/${id}/my-entry?userId=${userId}`, { cache: 'no-store' }),
    fetch(`${SERVER}/api/contests/${id}`, { cache: 'no-store' }),
    fetch(`${SERVER}/api/users/${userId}`, { cache: 'no-store' }),
  ]);

  if (!entryRes.ok || !contestRes.ok) notFound();

  const entry = await entryRes.json() as Entry;
  const contest = await contestRes.json() as ContestSummary;
  const user = userRes.ok ? (await userRes.json() as { id: string; handle: string }) : null;

  const displayHandle = entry.anonymous ? null : (user?.handle ?? null);
    ...
    <main className="min-h-screen bg-mist-gray">
      ...
      <div className="bg-soft-white border-b border-smoke">
        <div className="max-w-2xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          {isResolved && (
            <span
              className="text-xs font-semibold text-pewter border border-smoke px-2 py-1"
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        <div className="mb-8">
          ...
          >
            {displayHandle ? `${displayHandle}'s` : 'Anonymous'}<br />Betslip.
          </h1>
          <p className="text-graphite text-sm mt-2">{contest.name}</p>
        </div>
        ...
        <div className="bg-soft-white border border-smoke p-8 mb-6" style={{ borderRadius: 8 }}>
          {entry.finalRank !== null && (
            <div className="flex items-center gap-2 mb-4">
              <span
                className="text-xs font-semibold text-midnight-ink bg-signal-yellow px-2 py-0.5"
                style={{ borderRadius: 4 }}
              >
          ...
          <p className="text-xs font-semibold uppercase tracking-widest text-pewter mb-2">
            {hasResults ? 'Oddscore' : 'Oddscore — live after kickoff'}
          </p>
          <p
                  ...
                  {p.result ? (
                    <span
                      className={`shrink-0 text-xs font-semibold px-2 py-1 ${
                        correct
                          ? 'bg-signal-yellow text-midnight-ink'
                          : 'bg-smoke text-pewter'
                      }`}
                      style={{ borderRadius: 4 }}
                    >
                    ...
                    <span className="shrink-0 text-xs text-pewter">Pending</span>
                  )}
                </div>
              </div>
            );
        ...
        {entry.anonymous && (
          <p className="text-pewter text-xs mt-6 text-center">
            This entry was submitted anonymously.
          </p>
        )}
      </div>
    </main>
